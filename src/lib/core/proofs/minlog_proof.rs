...
wrapper_enum::wrapper_enum! {
    ...
    pub fwd bnd trait ProofBody: PrettyPrintable + Clone + PartialEq + Eq + Hash {
        pub fwd fn proved_formula(&self) -> Arc<MinlogPredicate>
        
        pub fwd fn length(&self) -> usize {
            1
        }
        
        pub fwd fn normalize(&self, eta: bool, pi: bool) -> Arc<MinlogProof>
        
        pub fwd fn unfold(&self) -> Arc<MinlogProof>
        
        pub fwd fn extracted_term(&self) -> Option<MinlogTerm>
        
        pub fwd fn get_type_variables(&self) -> IndexSet<Arc<MinlogType>> {
            IndexSet::new()
        }
        
        pub fwd fn get_algebra_types(&self) -> IndexSet<Arc<MinlogType>> {
            IndexSet::new()
        }
        ...
        pub fwd fn get_predicate_variables(&self) -> IndexSet<Arc<MinlogPredicate>> {
            IndexSet::new()
        }
        
        pub fwd fn get_comprehension_terms(&self) -> IndexSet<Arc<MinlogPredicate>> {
            IndexSet::new()
        }
        
        pub fwd fn get_inductive_predicates(&self) -> IndexSet<Arc<MinlogPredicate>> {
            IndexSet::new()
        }
        
        pub fwd fn get_prime_formulas(&self) -> IndexSet<Arc<MinlogPredicate>> {
            IndexSet::new()
        }
        
        pub fwd fn get_goals(&self) -> IndexSet<Arc<MinlogProof>> {
            IndexSet::new()
        }
        
        pub fwd fn get_assumptions(&self) -> IndexSet<Arc<MinlogProof>> {
            IndexSet::new()
        }
        
        pub fwd fn get_axioms(&self) -> IndexSet<Arc<MinlogProof>> {
            IndexSet::new()
        }
        
        pub fwd fn get_theorems(&self) -> IndexSet<Arc<MinlogProof>> {
            IndexSet::new()
        }
        
        pub fwd fn substitute(&self, from: &ProofSubstEntry, to: &ProofSubstEntry) -> Arc<MinlogProof>
        
        pub fwd fn first_conflict_with(&self, other: &Arc<MinlogProof>) -> Option<(ProofSubstEntry, ProofSubstEntry)>
        
        pub fwd fn match_with(&self, instance: &Arc<MinlogProof>) -> MatchOutput<ProofSubstEntry>
    }
...
impl MinlogProof {
    ...
    pub fn contains_goal(&self, goal: &Arc<MinlogProof>) -> bool {
        goal.is_goal() && self.get_goals().contains(goal)
    }
    
    pub fn contains_assumption(&self, asm: &Arc<MinlogProof>) -> bool {
        asm.is_assumption() && self.get_assumptions().contains(asm)
    }
    
    pub fn contains_axiom(&self, axm: &Arc<MinlogProof>) -> bool {
        axm.is_axiom() && self.get_axioms().contains(axm)
    }
    
    pub fn contains_theorem(&self, thm: &Arc<MinlogProof>) -> bool {
