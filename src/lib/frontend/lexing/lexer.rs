...
impl Lexer {
    ...
    pub fn lex(&self) -> LexingResult<Vec<Token>> {
        ...
        let source_borrow = source.read().unwrap();
...
impl<'src> Tokenizer<'src> {
    ...
    fn skip_comment(&mut self) -> LexingResult<()> {
        self.skip_line_comment()?;
        self.skip_block_comment()?;
        Ok(())
    }
    
    fn skip_line_comment(&mut self) -> LexingResult<()> {
    ...
    fn skip_block_comment(&mut self) -> LexingResult<()> {
        if self.check_match("/*", false) {
